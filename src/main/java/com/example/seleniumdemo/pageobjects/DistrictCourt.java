...
import com.example.seleniumdemo.pages.*;
...
public class DistrictCourt {
    ...
    private void navigateToDistrict(String state, String district) throws Exception {
        try {
            ...
            if (!driver.findElements(By.xpath("//*[@id=\"block-views-india-block\"]/div/div/div/ul/li[" + state +
                    "]/a")).isEmpty()) {
                System.out.println("Trying by State Value");
                we.findElement(By.xpath("//*[@id=\"block-views-india-block\"]/div/div/div/ul/li[" + state + "]/a")).click();
            } else {
                System.out.println("Trying by State Name");
                for (int i = 1; i <= driver.findElements(By.xpath("//*[@id=\"block-views-india-block\"]/div/div/div" +
                        "/ul/li/a")).size(); i++) {
                    if (driver.findElement(By.xpath("//*[@id=\"block-views-india-block\"]/div/div/div/ul/li[" + i +
                            "]/a")).getText().toLowerCase().contains(state.toLowerCase())) {
                        we.findElement(By.xpath("//*[@id=\"block-views-india-block\"]/div/div/div/ul/li[" + i + "]/a")).click();
                        break;
                    }
                }
            }
            ...
            if (!driver.findElements(By.xpath("//*[@id=\"map-position-list\"]/div/div/div[2]/div/div/div/ul/li[" + district + "]/a")).isEmpty()) {
                System.out.println("Trying by District Value");
                we.findElement(By.xpath("//*[@id=\"map-position-list\"]/div/div/div[2]/div/div/div/ul/li[" + district + "]/a")).click();
            } else {
                System.out.println("Trying by District Name");
                for (int i = 1; i <= driver.findElements(By.xpath("//*[@id=\"map-position-list\"]/div/div/div[2]/div/div/div/ul/li/a")).size(); i++) {
                    if (driver.findElement(By.xpath("//*[@id=\"map-position-list\"]/div/div/div[2]/div/div/div/ul/li" +
                            "[" + i + "]/a")).getText().toLowerCase().contains(district.toLowerCase())) {
                        we.findElement(By.xpath("//*[@id=\"map-position-list\"]/div/div/div[2]/div/div/div/ul/li" +
                                "[" + i + "]/a")).click();
                        break;
                    }
                }
    ...
    public int caseNumber(JsonBodyInputCaseNumber input) throws Exception {
        ...
        try {
            ...
            for (WebElement temp : le) {
                if (temp.getAttribute("value").contains(input.getBench_val())) {
            ...
            if (ret) {
                for (WebElement temp : le) {
                    if (temp.getText().contains(input.getBench_val())) {
    ...
    private void saveRecord(JsonBodyOutput j, boolean b) throws ParseException {
        ...
        if (!driver.findElements(CasePage.caseHistory).isEmpty()) {
            ...
            for (int i = 0; i < weList.size(); i++) {
                ...
                history.put("purpose", we.getText());
