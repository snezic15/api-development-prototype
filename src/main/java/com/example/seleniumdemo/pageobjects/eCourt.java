...
import com.example.seleniumdemo.jsonbody.JsonBodyInputCNRNumber;
import com.example.seleniumdemo.jsonbody.JsonBodyOutput;
import com.example.seleniumdemo.jsonbody.JsonBodyOutputToCaseNumber;
import com.example.seleniumdemo.pages.eCourtPage;
...
//        }
        ...
        //Table 2: Case Status
        if (!driver.findElements(eCourtPage.hearing).isEmpty()) {
            we = w.until(ExpectedConditions.presenceOfElementLocated(eCourtPage.hearing));
        ...
        if (!driver.findElements(eCourtPage.caseHistory).isEmpty()) {
            ...
            for (int i = 0; i < weList.size(); i++) {
                ...
                history.put("purpose", we.getText().trim());
